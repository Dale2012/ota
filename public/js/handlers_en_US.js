...
function uploadStart(file) {
	...
	return true;
        ...
        progress.setError();
        progress.toggleCancel(false);
        progress.setStatus("Pin Code can not be null." );
...
//		progress.setComplete();  progress.toggleCancel(false);
        //alert(serverData);
		progress.setStatus("File upload Complete and sending install address to device.");
...
//                var queryString = "file_key=" +fileUniqKey+"&file_name=" +file.name+"&file_size=" +file.size+"&file_type=" + file.type+"&pinCode=" +pingCode;
                ...
                $.ajax({
                    ...
                    success:function(resp){
                        if(resp.msg =='ok'){
                            progress.setStatus("Send Success.");
                        }else if(resp.msg =='invalid'){
                            progress.setStatus("The file is invalid.");
                        }else{
                            progress.setStatus("Send failure.");
                        }
